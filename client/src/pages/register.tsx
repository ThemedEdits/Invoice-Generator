...
import { Eye, EyeOff, Loader2, ArrowRight, Mail, Lock, Check, X, ArrowLeft, MailCheck, RefreshCw } from "lucide-react";
...
export default function Register() {
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    if (strength.score < 2)   { toast({ title: "Password too weak", description: "Please use a stronger password.", variant: "destructive" }); return; }
    setLoading(true);
    try {
        ...
        <div className="font-dm min-h-screen bg-[#0d0f14] flex items-center justify-center relative overflow-hidden p-6">
          ...
          <div className="w-full max-w-md relative z-10">
            ...
            <div className="glass-card border border-white/[0.08] rounded-2xl p-10 text-center animate-fadeUp">
              ...
              <h1 className="font-cinzel text-2xl font-bold text-white mb-2">Check your inbox</h1>
  ...
  return (
    <>
      ...
      <div className="font-dm min-h-screen bg-[#0d0f14] flex relative overflow-hidden">
        ...
        {/* Left panel */}
        <div className="hidden lg:flex lg:w-[42%] relative flex-col justify-between p-12 border-r border-white/[0.08]">
          ...
          <div>
            <h2 className="font-cinzel text-2xl font-bold text-white leading-snug mb-6">
              Start your journey<br /><span className="text-amber-400">to effortless invoicing.</span>
            </h2>
        ...
        <div className="flex-1 flex items-center justify-center p-6 relative z-10">
          <div className="w-full max-w-md">
              ...
              <form onSubmit={handleRegister} className="space-y-4">
                ...
                <div className="space-y-1.5">
                  ...
                  <div className={`relative flex items-center rounded-xl border transition-all duration-200 ${focused === "password" ? "border-amber-400/60 bg-amber-400/5" : "border-white/10 bg-white/[0.04]"}`}>
                    ...
                    <input type={showPass ? "text" : "password"} required value={password} onChange={e => setPassword(e.target.value)} onFocus={() => setFocused("password")} onBlur={() => setFocused(null)} placeholder="Create a strong password" className="w-full bg-transparent pl-10 pr-12 py-3 text-sm text-white placeholder:text-slate-400 outline-none" />
                      ...
                      <div className="flex gap-1">
                        {[1,2,3,4].map(i => (
                          <div key={i} className="flex-1 h-1 rounded-full bg-white/[0.08] overflow-hidden">
                            <div className={`h-full rounded-full strength-bar ${i <= strength.score ? strength.barColor : "bg-transparent"}`} style={{ width: i <= strength.score ? "100%" : "0%" }} />
                          </div>
                        ))}
                      </div>
                        ...
                        {RULES.map(rule => {
                          ...
                          return (
                            <div key={rule.label} className="flex items-center gap-1.5">
                              {ok ? <Check className="w-3 h-3 text-emerald-400 flex-shrink-0" /> : <X className="w-3 h-3 text-slate-600 flex-shrink-0" />}
                              <span className={`text-[10px] ${ok ? "text-emerald-400" : "text-slate-500"}`}>{rule.label}</span>
                            </div>
                          );
                        })}
                      </div>
                ...
                <div className="space-y-1.5">
                  ...
                  <div className={`relative flex items-center rounded-xl border transition-all duration-200 ${passMismatch ? "border-red-500/50 bg-red-500/5" : passMatch ? "border-emerald-500/50 bg-emerald-500/5" : focused === "confirm" ? "border-amber-400/60 bg-amber-400/5" : "border-white/10 bg-white/[0.04]"}`}>
                    <Lock className={`w-4 h-4 absolute left-3.5 transition-colors ${passMismatch ? "text-red-400" : passMatch ? "text-emerald-400" : focused === "confirm" ? "text-amber-400" : "text-slate-400"}`} />
                    <input type={showConf ? "text" : "password"} required value={confirm} onChange={e => setConfirm(e.target.value)} onFocus={() => setFocused("confirm")} onBlur={() => setFocused(null)} placeholder="Re-enter your password" className="w-full bg-transparent pl-10 pr-12 py-3 text-sm text-white placeholder:text-slate-400 outline-none" />
                    <button type="button" onClick={() => setShowConf(v => !v)} className="absolute right-3.5 text-slate-400 hover:text-slate-300 transition-colors" tabIndex={-1}>
                ...
                <div className="flex items-start gap-3 pt-1">
                  <button type="button" onClick={() => setAgreed(v => !v)} className={`w-5 h-5 rounded-md border flex items-center justify-center flex-shrink-0 mt-0.5 transition-all duration-150 ${agreed ? "bg-amber-400 border-amber-400" : "border-white/20 bg-white/[0.04] hover:border-amber-400/40"}`}>
                    {agreed && <Check className="w-3 h-3 text-slate-900" />}
                  </button>
                ...
                {/* Submit */}
                <button type="submit" disabled={loading || !agreed || passMismatch} className="w-full flex items-center justify-center gap-2 bg-amber-400 hover:bg-amber-300 disabled:opacity-50 disabled:cursor-not-allowed text-slate-900 font-bold py-3 rounded-xl transition-all duration-200 shadow-lg shadow-amber-400/20 hover:-translate-y-px active:translate-y-0 text-sm mt-1">
                  {loading
            ...
            <p className="mt-6 text-center text-sm text-slate-500 animate-fadeIn" style={{ animationDelay: "0.35s", animationFillMode: "both" }}>
              ...
              <Link href="/login"><a className="text-amber-400 font-semibold hover:text-amber-300 transition-colors">Sign in</a></Link>
            </p>
          </div>
        </div>
      </div>
